...
def convert_single_example(
    example_index, example, label_size, max_seq_length, tokenizer, max_qa_length
):
    ...
    if isinstance(example, classifier_utils.PaddingInputExample):
        ...
        if example_index < 5:
            ...
            for choice_idx, (tokens, input_ids, input_mask, segment_ids) in enumerate(
                zip(all_input_tokens, all_input_ids, all_input_mask, all_segment_ids)
            ):
...
def file_based_convert_examples_to_features(
    examples, label_list, max_seq_length, tokenizer, output_file, max_qa_length
):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_single_example(
            ex_index, example, len(label_list), max_seq_length, tokenizer, max_qa_length
        )
...
def create_model(
    ...
    token_type_ids = tf.reshape(
        segment_ids, [bsz_per_core * num_labels, max_seq_length]
    )
