...
def get_mlm_logits(model, albert_config, mlm_positions):
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        logits = tf.matmul(input_tensor, model.get_embedding_table(), transpose_b=True)
...
def module_fn(is_training):
    ...
    vocab_file = tf.constant(value=vocab_file_path, dtype=tf.string, name="vocab_file")
    ...
    hub.add_signature(
        ...
        inputs={},
        outputs=dict(
            vocab_file=vocab_model, do_lower_case=tf.constant(FLAGS.do_lower_case)
        ),
    )
