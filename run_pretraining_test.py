...
def _create_record(max_predictions_per_seq, max_seq_length, vocab_size):
    ...
    example.features.feature["masked_lm_positions"].int64_list.value.extend(
        [random.randint(0, max_seq_length - 1) for _ in range(max_predictions_per_seq)]
    )
    ...
    example.features.feature["masked_lm_weights"].float_list.value.extend(
        [1.0 for _ in range(max_predictions_per_seq)]
    )
...
def _create_input_file(
    filename, max_predictions_per_seq, max_seq_length, vocab_size, size=1000
):
...
class RunPretrainingTest(tf.test.TestCase):
    ...
    def test_pretraining(self):
        ...
        _create_config_file(FLAGS.albert_config_file, FLAGS.max_seq_length, vocab_size)
        _create_input_file(
            FLAGS.input_file,
