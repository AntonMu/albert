...
flags.DEFINE_string("train_feature_file", None, "training feature file.")
...
def validate_flags_or_throw(albert_config):
    ...
    if not FLAGS.do_train and not FLAGS.do_predict and not FLAGS.export_dir:
        err_msg = (
            "At least one of `do_train` or `do_predict` or `export_dir`"
            + "must be True."
        )
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        if tf.gfile.Exists(FLAGS.predict_feature_file) and tf.gfile.Exists(
            ...
            tf.logging.info(
                "Loading eval features from {}".format(FLAGS.predict_feature_left_file)
            )
        ...
        def get_result(checkpoint):
            ...
            for result in estimator.predict(
                predict_input_fn, yield_single_examples=True, checkpoint_path=checkpoint
            ):
            ...
            return (
                squad_utils.evaluate_v1(prediction_json, predictions),
                int(global_step),
            )
