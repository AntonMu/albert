...
flags.DEFINE_string(
    "train_file",
    None,
    "path to preprocessed tfrecord file. " "The file will be generated if not exst.",
)
...
flags.DEFINE_integer("num_keep_checkpoint", 5, "maximum number of keep checkpoints")


flags.DEFINE_bool(
    "high_only", False, "Whether to only run the model on the high school set."
)

flags.DEFINE_bool(
    "middle_only", False, "Whether to only run the model on the middle school set."
)
...
flags.DEFINE_integer("train_step", 12000, "Total number of training epochs to perform.")

flags.DEFINE_integer(
    "warmup_step", 1000, "number of steps to perform linear learning rate warmup for."
)
