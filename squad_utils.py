...
def accumulate_predictions_v1(
    result_dict, all_examples, all_features, all_results, n_best_size, max_answer_length
):
    ...
    for article in dataset:
        for p in article["paragraphs"]:
            for qa in p["qas"]:
                qid = qa["id"]
                gold_answers = [
                    a["text"] for a in qa["answers"] if normalize_answer_v2(a["text"])
                ]
                if not gold_answers:
...
def create_v2_model(
    ...
    with tf.variable_scope("end_logits"):
        if is_training:
            ...
            start_index = tf.one_hot(
                start_positions, depth=max_seq_length, axis=-1, dtype=tf.float32
            )
            ...
            start_index = tf.one_hot(
                start_top_index, depth=max_seq_length, axis=-1, dtype=tf.float32
            )
            start_features = tf.einsum("lbh,bkl->bkh", output, start_index)
            end_input = tf.tile(output[:, :, None], [1, 1, start_n_top, 1])
            start_features = tf.tile(start_features[None], [max_seq_length, 1, 1, 1])
    ...
    with tf.variable_scope("answer_class"):
        ...
        cls_index = tf.one_hot(
            tf.zeros([bsz], dtype=tf.int32), max_seq_length, axis=-1, dtype=tf.float32
        )
        ...
        ans_feature = tf.layers.dropout(ans_feature, dropout_prob, training=is_training)
